    ...
    <div className="flex flex-col md:flex-row gap-3 md:gap-4 lg:gap-6">
      ...
      <div className="min-w-0 flex-1 order-last md:order-none">
        <div className="bg-navi-card border border-navi-border rounded-xl p-4 sm:p-5 mb-4">
          <h2 className="text-xs font-semibold text-navi-muted mb-2">골 / 도움 기록</h2>
              ...
              <table className="w-full border-collapse text-xs">
                <thead>
                  <tr>
                    ...
                    <th className="text-left py-1.5 px-2 text-navi-muted font-semibold whitespace-nowrap">선수</th>
                    <th className="text-left py-1.5 px-2 text-navi-muted font-semibold whitespace-nowrap">골</th>
                    <th className="text-left py-1.5 px-2 text-navi-muted font-semibold whitespace-nowrap">도움</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredLogs.map((row, idx) => (
                    <tr key={idx} className="hover:bg-black/5">
                      <td className="py-1.5 px-2 text-navi-muted whitespace-nowrap">{formatDate(row.matchDate)}</td>
                      <td className="py-1.5 px-2 whitespace-nowrap">{row.opponent || '-'}</td>
                      <td className="py-1.5 px-2 font-semibold whitespace-nowrap">{row.playerName}</td>
                      <td className="py-1.5 px-2 whitespace-nowrap"><strong>{row.goals}</strong></td>
                      <td className="py-1.5 px-2 whitespace-nowrap">{row.assists}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
